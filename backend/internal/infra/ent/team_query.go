...
func (_q *TeamQuery) QueryStation() *StationQuery {
...
func (_q *TeamQuery) QueryContests() *ContestQuery {
...
func (_q *TeamQuery) WithStation(opts ...func(*StationQuery)) *TeamQuery {
	query := (&StationClient{config: _q.config}).Query()
...
func (_q *TeamQuery) WithContests(opts ...func(*ContestQuery)) *TeamQuery {
	query := (&ContestClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TeamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Team, error) {
	...
	if query := _q.withStation; query != nil {
		if err := _q.loadStation(ctx, query, nodes, nil,
			func(n *Team, e *Station) { n.Edges.Station = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withContests; query != nil {
		if err := _q.loadContests(ctx, query, nodes,
			func(n *Team) { n.Edges.Contests = []*Contest{} },
			func(n *Team, e *Contest) { n.Edges.Contests = append(n.Edges.Contests, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TeamQuery) loadStation(ctx context.Context, query *StationQuery, nodes []*Team, init func(*Team), assign func(*Team, *Station)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Team)
	for i := range nodes {
		if nodes[i].team_station == nil {
			continue
		}
		fk := *nodes[i].team_station
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "team_station" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TeamQuery) loadContests(ctx context.Context, query *ContestQuery, nodes []*Team, init func(*Team), assign func(*Team, *Contest)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(team.ContestsTable)
		s.Join(joinT).On(s.C(contest.FieldID), joinT.C(team.ContestsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(team.ContestsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(team.ContestsPrimaryKey[1]))
